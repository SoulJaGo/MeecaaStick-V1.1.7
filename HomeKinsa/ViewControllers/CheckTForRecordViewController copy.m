...
@interface CheckTForRecordViewController ()
{
    //录音器
...
//    NSLog(@"%@", myData);
    ...
    short *result = malloc(2*1024*sizeof(short));
...
//        NSLog(@"the number is %d, hex form: %x", number, number);
    ...
    mk_AudioFormat(16000, 1000, 1, 0, 0, 0, 0);
    printf("result %i",mk_Data(result, 2*1024, pdestdata, &bytelen));
    for(int k = 0; k < 5;k++)
    {
...
//    self.label1.text= str;
    ...
    NSString *resultStr = @"";
    if((pdestdata[1] + pdestdata[2])==pdestdata[3])
    {
        resultStr = [NSString stringWithFormat:@"测试温度结果：%@", str1];
...
//    NSLog(@"Average input: %f Peak input: %f Low pass results: %f", [recorder averagePowerForChannel:0], [recorder peakPowerForChannel:0], lowPassResults);
    ...
    NSLog(@"%i", recordCount);
    if (recordCount>=90) {   //修改了此处加大了录音部分
        recordCount = 0;
        [self upAction:nil];
    }
    recordCount++;
