...
int Decoder::XcorrDecode(INT16* temperature, INT16* recordBuf, INT32 syncPeakLocation)
{
    ...
    INT32 maxValueLocations[4];
    INT32 sortIndex[4];
    
    INT32 correctXcorrPeakLocation;
    INT32 maxXcorrLocationThHi;
    INT32 maxXcorrLocationThLow;
    ...
    // claer the symbols to default status
    for (int symbolId = 0; symbolId < symbolNum; ++symbolId)
    {
        symbol[symbolId].ResetSymbols();
    }
    ...
    for (int symbolId = 0; symbolId < symbolNum; ++symbolId)
    {
        ...
        if (nCurDecodeLocation >= infoPoints - maxClkSkewTh)
            break;
        ...
        maxValueLocations[0] = maxXcorrSymbol00Location;
        maxValueLocations[1] = maxXcorrSymbol01Location;
        maxValueLocations[2] = maxXcorrSymbol10Location;
        maxValueLocations[3] = maxXcorrSymbol11Location;
        ...
        symbol[symbolId].active = 1;
        
        // If the maxim xcorr value's maxim value location in compare range is far way from the correct xcorr peak,
        // while the second maxim xcorr value's maxim value location in compare range is near the correct xcorr peak,
        // swap the maxim and second maxim value
        
        // Do not need to plus 1 in C
        correctXcorrPeakLocation = maxClkSkewTh;
        
        maxXcorrLocationThHi = maxClkSkewTh - 1;
        maxXcorrLocationThLow = 2;
        
        if ((abs(maxValueLocations[sortIndex[3]] - correctXcorrPeakLocation) >= maxXcorrLocationThHi) && (abs(maxValueLocations[sortIndex[2]] - correctXcorrPeakLocation) <= maxXcorrLocationThLow))
        {
            if (sortIndex[2] == 0)
            {
            ...
            // Second max : max rate
            symbol[symbolId].secondMaxToMaxRate = (double)maxValues[3] / maxValues[2];
    ...
    if (firstStartLocation == -1)
        return CANNOT_FIND_START_SYMBOL_ERR;
    ...
    // Check whether the last bit is active
    // if the last bit is active, all previous bits are all active
    if (symbol[exportLocation - 1].active == 0)
        return INFO_NOT_COMPLETED_ERR;
    
    for (int byteId = 0; byteId < 4; ++byteId)
    {
        // start bit is not 0
        if (decodedArray[byteId][0] != 0)
            return START_SYMBOL_NOT_ZERO_ERR;
        
        // stop bits are not 1
        if ((decodedArray[byteId][BITS_PER_FRAME-2] != 1) || (decodedArray[byteId][BITS_PER_FRAME - 1] != 1))
            return STOP_SYMBOL_NOT_ONE_ERR;
    ...
    // In java, char is 2 bytes. So need to consider this.
    checkSum = (dataArray[0] + dataArray[1])%256;
    
    if (dataArray[2] != checkSum)
        return CHECKSUM_ERR;
    
    if (0x55 != dataArray[3])
        return FOURTH_BYTE_ERR;
