...
#define RSA_PUBLIC_KEY_BASE64 @"MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDEK5QYIiENNKBtQjYco8mDkMTCWzOrtPPKKgOt8iEHnbPjhPZ0iL6H7MSwXHEMFKicpwQUrONvRb7SbGT5leS7CMk9js+zEh+fHFiJD00pDoiCn4lqj0I2Vyc6V+D5ukdxwUzstnf30jwQRT85AtSl4CggboQWlDy6nchEEiAEYQIDAQAB"
...
- (NSString *)encryptAES128WithKey:(NSString *)key {
    NSData *data = [self dataUsingEncoding:NSUTF8StringEncoding];
//    data = [data encryptAES128WithKey:key];
//    return [data encodeBase64];
    return @"";
}
...
- (NSString *)decryptAES128WithKey:(NSString *)key {
    NSData *data = [self decodeBase64];
//    data = [data decryptAES128WithKey:key];
    if (!data) return nil;
...
- (NSData *)decodeBase64 {    
    ...
    while ( ((intCurrent = *objPointer++) != '\0') && (intLength-- > 0) ) {
        ...
        intCurrent = base64DecodingTable[intCurrent];
        if (intCurrent == -1) {
...
- (NSUInteger)indexOfCharacter:(unichar)character {
    return [self indexOfCharacter:character from:0];
}
- (NSUInteger)indexOfCharacter:(unichar)character from:(NSUInteger)fromIndex {
    NSUInteger len = [self length];
    NSUInteger p = fromIndex;
    while (p < len) {
        unichar c = [self characterAtIndex:p];
        if (c == character) return p;
        p++;
    }
    return NSNotFound;
}
...
- (NSString *)encryptRSAWithKey:(SecKeyRef) key{
    ...
    for (int i=0; i<blockCount; i++) {
        unsigned long bufferSize = MIN(blockSize,[stringBytes length] - i * blockSize);
